...
export const Contact = () => {
  ...
  return (
    <Container as="section" className={[styles.contactCont, "generics"].join(" ")} id="contact">
      <div className={[styles.contact, styles.section].join(' ')}>
        <Title className={styles.title}>Contact Information</Title>
        <BoxFromTop className={[styles.contactGridBox, "card"].join(" ")} delayProp={0.1}>
          <Grid columns={2} centered className={styles.contactGrid} relaxed='very' doubling>
            ...
            <Divider className={styles.hDivide} horizontal>
              <p>Or</p>
            </Divider>
            <Grid.Column className={styles.contactGridCols}>
                  ...
                  <Grid className={styles.socialGrid} centered>
                    {
                      myContacts.map((item, index)=>(
                        <Grid.Column key={index} className={styles.socialCols} computer={3} largeScreen={3} tablet={3} mobile={3}> 
                            <motion.div whileHover={{ scale: 1.1, y:-3,transition: { duration: 0.1 } }}>
                              <a href={item.link} target='_blank' className={[styles.link, 'darkedLink'].join(" ")}>{item.icon}</a>
                            </motion.div>
                        </Grid.Column>
                      ))
                    }
